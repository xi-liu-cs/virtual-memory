        ...
        for(uintptr_t i = PROC_START_ADDR; i < MEMSIZE_VIRTUAL; i += PAGESIZE)
        {
            ...
            if((vm.pn != -1) && (vm.perm & PTE_U) && (vm.pa != (uintptr_t)console))
            {
                if(vm.perm & PTE_W)
                {
                    ...
                    virtual_memory_map(processes[child_pid].p_pagetable,
                        i, 
                        p,
                        PAGESIZE,
                        vm.perm,
